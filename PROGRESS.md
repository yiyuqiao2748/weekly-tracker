...
最后更新：2026-09-21（深夜）。出图重做成图册（多周连排 / 按人成册 / 图纸目录 / 配平）已提交并 push，占位页那一条修的是线上冒烟撞出来的空册分支。版面留档还没补。

## 当前状态

| | |
| --- | --- |
| 线上版本 | `e5eca4e`（图册版出图 + 占位页）。push 后轮询过 Pages，**服务内容与本地 `index.html` md5 一致**；这个比对口径每轮上线后都该做一次，命令见「怎么验证」 |
