...
md5 一致之后还可以补一层独立证据：`python3 .preview/live_smoke_sheet.py` 用无头浏览器打开真站点的 `?sheet` 直链，断言版面修复在部署产物里是活的，并把**每一条请求的方法**记下来——出现非 GET 就地失败。这台 headless profile 在 `github.io` 源下没有 Token，所以板子是 0 条任务，走的正是空周那条分支。
